...
export function createTaskCard(task: Task, taskClass: string, options: TaskCardOptions = {}): HTMLElement {
  ...
  if (task.noteColor && task.noteTextColor) {
    ...
    style = '--task-background:#7D7D7D33;--task-color:#7D7D7D;--dark-task-text-color:#4d4d4d;--light-task-text-color:#a8a8a8'
  }
  ...
  const noteDisplay = task.noteIcon ? task.noteIcon + ' ' + noteFilename : taskIcon + ' ' + noteFilename
  ...
  if (Platform.isMobile) {
    attachLongPress(div, (text) => { alert(text) })
  }
