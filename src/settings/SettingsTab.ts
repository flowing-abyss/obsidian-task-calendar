...
export class CalendarSettingsTab extends PluginSettingTab {
  constructor(app: App, private plugin: TaskCalendarPlugin) {
    super(app, plugin)
  }
  ...
  display(): void {
    ...
    containerEl.empty()
    containerEl.createEl('h2', { text: 'Task Calendar Settings' })
    ...
    new Setting(containerEl)
      ...
      .addText(t => t
        ...
        .onChange(async v => { this.plugin.settings.taskPrefix = v; await this.plugin.saveSettings() }),
      )
    ...
    new Setting(containerEl)
      ...
      .addToggle(t => t
        ...
        .onChange(async v => { this.plugin.settings.addToToday = v; await this.plugin.saveSettings(); this.display() }),
      )
    ...
    if (!this.plugin.settings.addToToday) {
      new Setting(containerEl)
        ...
        .addText(t => t
          ...
          .onChange(async v => { this.plugin.settings.customFilePath = v; await this.plugin.saveSettings() }),
        )
    }

    containerEl.createEl('h3', { text: 'Desktop defaults' })
    this.renderViewConfigSettings(containerEl, 'desktop')

    containerEl.createEl('h3', { text: 'Mobile defaults' })
  ...
  private renderViewConfigSettings(container: HTMLElement, platform: 'desktop' | 'mobile'): void {
    ...
    new Setting(container)
      ...
      .addDropdown(d => d
        ...
        .onChange(async v => { cfg.defaultView = v as typeof cfg.defaultView; await this.plugin.saveSettings() }),
      )
    ...
    new Setting(container)
      ...
      .addDropdown(d => {
        ...
        return d
          .setValue(cfg.style)
          .onChange(async v => { cfg.style = v; await this.plugin.saveSettings() })
      })
    ...
    new Setting(container)
      ...
      .addDropdown(d => d
        ...
        .onChange(async v => { cfg.firstDayOfWeek = parseInt(v) as typeof cfg.firstDayOfWeek; await this.plugin.saveSettings() }),
      )

    new Setting(container)
      .setName('Daily note folder')
      .addText(t => t
        .setValue(cfg.dailyNoteFolder)
        .onChange(async v => { cfg.dailyNoteFolder = v; await this.plugin.saveSettings() }),
      )
    ...
    new Setting(container)
      ...
      .addText(t => t
        .setValue(cfg.dailyNoteFormat)
        .onChange(async v => { cfg.dailyNoteFormat = v; await this.plugin.saveSettings() }),
      )
